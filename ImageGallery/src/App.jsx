...
const App = () => {
  ...
  return (
    <div>
      <h1 className="text-center shadow-lg p-5  text-lg">Image Gallery</h1>
      <div className="flex flex-col items-center">
        <p className="mt-4">Click on an image!</p>
        <div className="flex flex-wrap gap-2">
          {images.map((element, index) => (
            <div key={index} onClick={() => handleClick(index)}>
              <img src={element} className="w-70 rounded-2xl shadow-lg" />
            </div>
          ))}
        </div>
        ...
        {selectImage && (
          <div>
            <h1 className="text-center mt-5 font-bold">Selected Image</h1>
            <img src={selectImage} className="rounded-2xl shadow-lg" />
          </div>
        )}
      </div>
    </div>
